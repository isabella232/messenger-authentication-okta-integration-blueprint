  ...
  * Using OAuth 2.0 Endpoint.
   ...
   **a. Okta Sign-In using SDK:**
    ...
    **b. Okta Sign-In using Endpoint:**
    ...
    | Parameter | Description |
    | --------- |----------|
    | DomainURL | Authorization server's endpoint(https://{okta-user-domain}.okta.com/oauth2/default/v1/authorize)|
    | ClientId  | Specify the clientID which was generated when the Okta app was created. Refer [OKTA Set-Up](#okta-set-up)|
...
Genesys('registerPlugin', 'AuthProvider', (AuthProvider) => {
  ...
  e.resolve({
      ...
      nonce: <nonce>, // For Okta Sign-In using SDK approach, pass the random string value stored in session storage. For Sign-In using endpoint approach, generate and send a random string value
      maxAge: <maxAge> // pass elapsed time in seconds and it is an optional parameter
      codeVerifier: <codeVerifier> //  For PKCE flow : If Okta Sign-In using SDK approach is used, Pass code verifier from session storage. If Sign-In using endpoint approach is used, pass a cryptographically random string which was used previously to generate codeChallenge
    });
  });
});
```
...
3. Enter the Client credentials to Log in to messenger.
